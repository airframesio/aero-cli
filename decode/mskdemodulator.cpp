...
MskDemodulator::MskDemodulator(QObject *parent) : QObject(parent) {
  ...
  ebnomeasure = new MSKEbNoMeasure(
      2.0 * Fs); // 2 second ave //SamplesPerSymbol*125);//125 symbol averaging
...
void MskDemodulator::setSettings(Settings _settings) {
  ...
  bbcycbuff_ptr = 0;
  bbtmpbuff.resize(bbnfft);
  coarsefreqestimate->setSettings(_settings.coarsefreqest_fft_power, lockingbw,
                                  fb, Fs);
  ...
  delete ebnomeasure;
  ebnomeasure = new MSKEbNoMeasure(
      2.0 * Fs); // 1 second ave //SamplesPerSymbol*125);//125 symbol averaging
...
void MskDemodulator::CenterFreqChangedSlot(
...
qint64 MskDemodulator::writeData(const char *data, qint64 len) {
  ...
  for (int i = 0; i < (int)(len / sizeof(short)); i++) {
    ...
    if ((coarseCounter >= Fs || !cpuReduce)) {
      ...
      if (bbcycbuff_ptr % (cpuReduce ? bbnfft : bbnfft / 4) ==
          0) // 75% overlap if not in low cpu mode
      {
        for (int j = 0; j < bbcycbuff.size(); j++) {
    ...
    if (!dcd) {
      st_osc.AdvanceFractionOfWave(-(1.0 - weighting) * st_angle_error *
                                   (0.05 / 360.0));
    ...
    if (st_osc.IfHavePassedPoint(ee)) {
      ...
      if (dcd)
        carrier_aggression = 8.0 * correctionfactor;
...
void MskDemodulator::FreqOffsetEstimateSlot(
...
void MskDemodulator::DCDstatSlot(bool _dcd) { dcd = _dcd; }
