...
int RISUData::findisuitem(RISUItem &anisuitem) {
  if (anisuitem.SUTYPE > 11)
    return -1;
  if (anisuitem.SUTYPE < 1)
    return -1;
  for (int i = 0; i < isuitems.size(); i++) {
    if ((anisuitem.GESID == isuitems[i].GESID) &&
        (anisuitem.AESID == isuitems[i].AESID) &&
...
bool RISUData::update(QByteArray data) {
  ...
  int BytesInSU = 0;
  if ((anisuitem.SUTYPE >= 1) && (anisuitem.SUTYPE <= 11))
    BytesInSU = anisuitem.SUTYPE;
  bool SignalingInfoSU = false;
  if (anisuitem.SUTYPE == 15)
    SignalingInfoSU = true;

  int thisnum = 11 * SUTotal - 11 + BytesInSU;
  if (thisnum > 0) {
    if (pisuitem->userdata.size() == 0)
      pisuitem->userdata.resize(thisnum);
    if (thisnum < pisuitem->userdata.size())
      pisuitem->userdata.resize(thisnum);
  }
  if (!SignalingInfoSU) {
    for (int i = 0 - 1 + 7; i < BytesInSU - 1 + 7; i++)
      pisuitem->userdata[i + 11 * SUindex + 1 - 7] = data[i];
    pisuitem->filledarray |= (1 << SUindex);
  } else
    pisuitem->userdata.clear();

  if ((SignalingInfoSU) || ((pisuitem->filledarray == 7) && (SUTotal == 3)) ||
      ((pisuitem->filledarray == 3) && (SUTotal == 2)) ||
      ((pisuitem->filledarray == 1) && (SUTotal == 1))) {
...
int ISUData::findisuitem71(ISUItem &anisuitem) {
  if (anisuitem.NOOCTLESTINLASTSSU > 8)
    return -1;
  for (int i = 0; i < isuitems.size(); i++) {
...
bool ISUData::update(QByteArray data) {
  ...
  case AEROTypeP::User_data_ISU_RLS_P_T_channel: {
    ...
    anisuitem.AESID = ((uchar)data[1]) << 8 * 2 | ((uchar)data[2]) << 8 * 1 |
                      ((uchar)data[3]) << 8 * 0;
    ...
    for (int i = 8; i <= 9; i++)
      anisuitem.userdata += data[i];

    int idx = findisuitem71(anisuitem);
    if (idx < 0)
      isuitems.push_back(anisuitem);
    else
      isuitems[idx] = anisuitem;

    // qDebug()<<(((QString)"71 isudata: AESID = %1 GESID = %2 QNO = %3 REFNO =
    // %4 SEQNO = %5 NOOCTLESTINLASTSSU =
    // %6").arg(anisuitem.AESID).arg(anisuitem.GESID).arg(anisuitem.QNO).arg(anisuitem.REFNO).arg(anisuitem.SEQNO).arg(anisuitem.NOOCTLESTINLASTSSU));
  } break;
  default: {
    ...
    (pisuitem->SEQNO)--;
    if (pisuitem->SEQNO == 0) {
      for (int i = 2; i <= (pisuitem->NOOCTLESTINLASTSSU + 1); i++)
        pisuitem->userdata += data[i];
      lastvalidisuitem = *pisuitem;
      // qDebug()<<"final ssu";
      return true;
    } else
    ...
    // qDebug()<<((QString)"").sprintf("ssu data: SEQNO %d, QNO %d, REFNO
    // %d",pisuitem->SEQNO,pisuitem->QNO,pisuitem->REFNO);
  } break;
  }
...
int ACARSDefragmenter::findfragment(ACARSItem &acarsitem) {
  ...
  for (int idx = 0; idx < acarsitemexts.size(); idx++) {
    ...
    if ((acarsitem.PLANEREG == pitem->anacarsitem.PLANEREG) &&
        (acarsitem.LABEL == pitem->anacarsitem.LABEL) &&
        (acarsitem.MODE == pitem->anacarsitem.MODE) &&
        // (acarsitem.TAK==pitem->anacarsitem.TAK)&& //i found once time TAK did
        // not match should we skip this check??
...
bool ACARSDefragmenter::defragment(ACARSItem &acarsitem) {
  ...
  if (idx < 0) {
    if (!acarsitem.moretocome)
      return true;
    anacarsitemext.anacarsitem = acarsitem;
    anacarsitemext.count = 0;
    acarsitemexts.push_back(anacarsitemext);
    // qDebug()<<"pushed first acars
    // fragment"<<anacarsitemext.anacarsitem.isuitem.AESID;
    return false;
  }
...
ParserISU::ParserISU(QObject *parent) : QObject(parent) {
  downlink = false;
  // dblookup
  dbtu = new DataBaseTextUser(this);
  connect(dbtu, SIGNAL(result(bool, int, QStringList)), this,
          SLOT(acarslookupresult(bool, int, QStringList)));
}
bool ParserISU::parse(ISUItem &isuitem) {
  ...
  for (int i = 0; i < isuitem.userdata.size(); i++) {
    ...
    parity &= 1;
    if (parity)
      parities.push_back(0x01);
    else
      parities.push_back(0x00);
    byte &= 0x7F;
    textish += byte;
  }
  ...
  // 15 C1 B6 51 (??? seem to have parity but not printable, i think this is
  // always 4 bytes) 02 (STX) (only if text is there) 2F D0 49 CB 43 D0 D9 C1 AE
  // C1 C4 D3 AE C8 C2 AD 4A C8 CD B0 37 B0 34 B0 C2 B0 B0 B0 43 B0 B0 B0 C4 B0
  // 31 B0 45 B0 31 31 B0 B0 B0 34 34 B0 C4 83 or 97 (ETX/ETB) * (ETB when text
  // is over 220 chars) 93 AB (BSC no parity bits) 7F (DEL) *

  /*QString str;
  for(int
  i=0;i<isuitem.userdata.size();i++)str+=(((QString)"%1").arg((uchar)isuitem.userdata[i],2,
  16, QChar('0'))).toUpper()   ;
  ...
  // not sure about some of these conditions, certainly missing some messages
  // with all of them on
  if ((isuitem.userdata.size() > 16) && ((uchar)isuitem.userdata[0]) == 0xFF &&
      ...
      && (((uchar)isuitem.userdata[15]) == 0x83 ||
          ((uchar)isuitem.userdata[15]) == 0x02))
    isacars = true;
  ...
  if (isacars) {
    ...
    if (((uchar)isuitem.userdata[15]) == 0x02)
      anacarsitem.hastext = true;
    ...
    for (int k = 4; k < 4 + 7; k++) {
      ...
      if (!parities[k]) {
        anerror = ((QString) "")
                      .asprintf("ISU: AESID = %X GESID = %X QNO = %02X REFNO = "
                               "%02X : Parity error",
                               isuitem.AESID, isuitem.GESID, isuitem.QNO,
                               isuitem.REFNO);
        emit Errorsignal(anerror);
        return false;
      }
    ...
    if (anacarsitem.hastext) {
      for (int k = 16; k < isuitem.userdata.size() - 1 - 3; k++) {
        ...
        if ((byte < 0x20) && (!(byte == 10 || byte == 13))) {
          //                    qDebug()<<"This is strange. byte<0x20
          //                    byte="<<byte;
        }
        if (byte == 0x7F)
          anacarsitem.message += "<DEL>";
        else
    ...
    if (acarsdefragmenter.defragment(anacarsitem)) {
      ...
      *pai = anacarsitem;
      QString AESIDstr =
          ((QString) "").asprintf("%06X", anacarsitem.isuitem.AESID);
  ...
  for (int i = 0; i < isuitem.userdata.size(); i++)
    anacarsitem.message +=
        (((QString) "%1").arg((uchar)isuitem.userdata[i], 2, 16, QChar('0')))
            .toUpper();
  anacarsitem.valid = true;
...
void ParserISU::setDataBaseDir(const QString &dir) { databasedir = dir; }
void ParserISU::acarslookupresult(bool ok, int ref, const QStringList &result) {
  ...
  if (panacarsitem == NULL)
    return;

  // remove prepending dots
  int i = 0;
  while ((i < panacarsitem->PLANEREG.size()) &&
         (panacarsitem->PLANEREG[i] == '.'))
    i++;
  panacarsitem->PLANEREG =
      panacarsitem->PLANEREG.right(panacarsitem->PLANEREG.size() - i);

  if (result.size() ==
      QMetaEnum::fromType<DataBaseTextUser::DataBaseSchema>().keyCount()) {
    // if non acars message then fill in reg with lookup value
    if (panacarsitem->nonacars) {
      panacarsitem->PLANEREG =
          result[DataBaseTextUser::DataBaseSchema::Registration].toLatin1();
    }
...
QVector<int> &AeroLInterleaver::deinterleaveMSK(QVector<int> &block,
                                                int blocks) {
  assert(block.size() >= (M * blocks));

  // we need to first deinterleave 5 cols for the first block then 3 cols for
...
QByteArray &AeroLInterleaver::deinterleaveMSK_ba(QVector<int> &block,
                                                 int blocks) {
  assert(block.size() >= (M * blocks));
...
bool OQPSKPreambleDetectorAndAmbiguityCorrection::setPreamble(
    quint64 bitpreamble1, quint64 bitpreamble2, int len) {
  if (len < 1 || len > 64)
    return false;
  preamble1.clear();
  preamble2.clear();
  ...
  for (int i = len - 1; i >= 0; i--) {
    if ((bitpreamble1 >> i) & 1)
      preamble1.push_back(1);
    else
      preamble1.push_back(0);

    if ((bitpreamble2 >> i) & 1)
      preamble2.push_back(1);
    else
      preamble2.push_back(0);
  }
  if (preamble1.size() < 1)
    preamble1.resize(1);
...
void OQPSKPreambleDetectorAndAmbiguityCorrection::setTollerence(
    int _tollerence) {
  tollerence = _tollerence;
}
...
AeroL::AeroL(QObject *parent) : QObject(parent) {
  ...
  // new viterbi decoder (this is a qobject and has us as perent so is deleted
  // automatically)
  ...
  preambledetector.setPreamble(
      3780831379LL, 32); // 0x3780831379,0b11100001010110101110100010010011

  // I	1010 1011 0011 0111 0110 1001 0011 1000 1011 1100 1010 0011 0000 =
  // 0xAB376938BCA30 hex = 3012071630031408 decimal Q	0000 1100 0101 0011 1101
  // 0001 1100 1001 0110 1110 1100 1101 0101 = 0xC53D1C96ECD5 hex =
  // 216866263330005 decimal
  ...
  preambledetectorphaseinvariantimag.setPreamble(
      3780831379LL, 32); // 0x3780831379,0b11100001010110101110100010010011
  preambledetectorphaseinvariantreal.setPreamble(
      3780831379LL, 32); // 0x3780831379,0b11100001010110101110100010010011

  // MSK burst preamble detector
  mskBurstDetector.setPreamble(
      3780831379LL, 32); // 0x3780831379,0b11100001010110101110100010010011
...
void AeroL::setBitRate(double fb) { setSettings(fb, burstmode); }

void AeroL::setBurstmode(bool _burstmode) { setSettings(ifb, _burstmode); }
...
void AeroL::setSettings(double fb, bool _burstmode) {
  ...
  case 600:
    ...
    AERO_SPEC_TotalNumberOfBits =
        AERO_SPEC_BitsInHeader + AERO_SPEC_NumberOfBits + 32;
    useingOQPSK = false;
    break;
  case 1200:
  ...
  case 8400:
    ...
    AERO_SPEC_TotalNumberOfBits =
        AERO_SPEC_BitsInHeader + AERO_SPEC_NumberOfBits;
    useingOQPSK = true;
    break;
  case 10500:
    ...
    AERO_SPEC_TotalNumberOfBits =
        AERO_SPEC_BitsInHeader + AERO_SPEC_NumberOfBits + 64;
    useingOQPSK = true;
    break;
  default:
    leaver.setSize(9); // 9 for 1200 bps, 6 for 600 bps
...
QByteArray &AeroL::Decode(QVector<short> &bits,
                          bool soft) // 0 bit --> oldest bit
{
  decodedbytes.clear();
  ...
  for (int i = 0; i < bits.size(); i++) {
    ...
    if (useingOQPSK) {
      ...
      // for 10500 UW should be about 80 samples after start of packet signal
      // from demodulator if not we have a false positive
    ...
    if (cntr < 16) {
      if (cntr == 0) {
        frameinfo = bit;
        infofield.clear();
        if (burstmode) // for R and T channels there are no headers so just fill
                       // in a dummy one
        {
          formatid = 1;
          supfrmaker = 0;
          ...
          if (rtchanneldeleavefecscram.resetblockptr() ==
              RTChannelDeleaveFECScram::Bad_Packet) {
    ...
    if (cntr == 15) {
      ...
      // delay frame info by one frame needed as viterbi decoder and delayline
      // make 1 frame delay
      quint16 tval = frameinfo;
      frameinfo = lastframeinfo;
      lastframeinfo = tval;

      // if(framecounter1!=framecounter2)decodedbytes.push_back("Error: Frame
      // Counter mismatch\n");
    ...
    if (cntr >= 16) {
      ...
      if (burstmode) {
        ...
        case RTChannelDeleaveFECScram::OK_R_Packet: {
            ...
            decline +=
                ((QString) " 0x%1")
                    .arg(((QString) "")
                             .asprintf(
                                 "%02X",
                                 (uchar)rtchanneldeleavefecscram.infofield[j]));
          }
          decline += " ";

          using namespace AEROTypeR;
          MessageType message =
              (MessageType)((uchar)rtchanneldeleavefecscram.infofield[2]);
          if ((((uchar)rtchanneldeleavefecscram.infofield[1]) & 0x08) == 0x08)
            message = User_data_ISU_SSU_R_channel;
          ...
          case User_data_ISU_SSU_R_channel:
            ...
            if (risudata.update(rtchanneldeleavefecscram.infofield.left(17))) {
              ...
              parserisu->parse(
                  risudata.lastvalidisuitem); // emits signals if it find valid
                                              // acars data or errors
            }
            ...
            decline += ((QString) " SU %1 of %2. AES: %3 GES: %4")
                           .arg(SUindex + 1)
                           .arg(SUTotal)
                           .arg((((QString) "%1").arg(AESID, 6, 16, QChar('0')))
                                    .toUpper())
                           .arg((((QString) "%1").arg(GES, 2, 16, QChar('0')))
                                    .toUpper());
            /*decline+=" \"";
                for(int m=6;m<17;m++)
                {
          ...
          decline += '\n';
          if ((message == User_data_ISU_SSU_R_channel) &&
              (donotdisplaysus.contains(0xC0)))
            decline.clear();
          else if (donotdisplaysus.contains(message))
            decline.clear(); // do not display these SUs as given to us by user
        ...
        case RTChannelDeleaveFECScram::OK_T_Packet: {
          ...
          int GES = byte4;
          decline +=
              ((QString) " T Packet from AES: %3 to GES: %4 with %5 SUs\n")
                  .arg((((QString) "%1").arg(AESID, 6, 16, QChar('0')))
                           .toUpper())
                  .arg((((QString) "%1").arg(GES, 2, 16, QChar('0'))).toUpper())
                  .arg(rtchanneldeleavefecscram.numberofsus);
          decodedbytes += decline.toUtf8();
          decline.clear();

          for (int k = 0; k < rtchanneldeleavefecscram.numberofsus; k++) {
            for (int j = 0; j < 12 - 2; j++) {
              decline += ((QString) " 0x%1")
                             .arg(((QString) "")
                                      .asprintf("%02X",
                                               (uchar)rtchanneldeleavefecscram
                                                   .infofield[6 + k * 12 + j]));
            }

            MessageType message = (MessageType)((uchar)rtchanneldeleavefecscram
                                                    .infofield[6 + k * 12]);
            if ((message & 0xC0) == 0xC0)
              message = User_data_ISU_SSU_T_channel;
            switch (message) {
            ...
            case User_data_ISU_RLS_T_channel:
              ...
              isudata.update(
                  rtchanneldeleavefecscram.infofield.mid(6 + k * 12, 10));
              break;
            case User_data_ISU_SSU_T_channel:
              ...
              if (isudata.update(
                ...
                parserisu->downlink = burstmode;
                parserisu->parse(
                    isudata.lastvalidisuitem); // emits signals if it find valid
                                               // acars data or errors
              } else if (isudata.missingssu)
                decline += " missing (or if unimplimented then TODO in C++)";
              break;
            }

            decline += "\n";

            if ((message == User_data_ISU_SSU_T_channel) &&
                (donotdisplaysus.contains(0xC0)))
              decline.clear();
            else if (donotdisplaysus.contains(message))
              decline.clear(); // do not display these SUs as given to us by
                               // user
        ...
        if (idx == (block.size() - 1)) // block is now filled
        {
          ...
          QVector<int> deconvol =
              jconvolcodec->Decode_Continuous(deleaveredblockBA);

          // delay line for frame alignment for non burst modes. This is needed
          // for the scrambler
          dl2.update(deconvol);
          ...
          if ((cntr - AERO_SPEC_BitsInHeader) ==
              (AERO_SPEC_NumberOfBits - 1)) // frame is done when this is true
          {
            ...
            QString decline;
            if (formatid != 1)
              decline += "format ID error\n";
            for (int k = 0; k < infofield.size() / 12; k++) {
              quint16 crc_calc =
                  crc16.calcusingbytes(&infofieldptr[k * 12], 12 - 2);
              quint16 crc_rec = (((uchar)infofield[k * 12 + 12 - 1]) << 8) |
                                ((uchar)infofield[k * 12 + 12 - 2]);

              if ((!crc_rec) && (crc_calc != crc_rec)) {
                int tsum = 0;
                for (int ii = 0; ii < (12 - 2); ii++)
                  tsum += (uchar)infofieldptr[k * 12 + ii];
                if (tsum == 0)
                  crc_calc = 0; // some sus are just zeros
              }

              // keep track of the DCD for non burst modes
              if (crc_calc == crc_rec) {
                if (datacdcountdown < 12)
                  datacdcountdown += 2;
              } else {
                if (datacdcountdown > 0)
                  datacdcountdown -= 3;
              ...
              decline += QChar(k + (char)'0'); // SU number in frame
              for (int j = 0; j < 12 - 2; j++) {
                decline += ((QString) " 0x%1")
                               .arg(((QString) "")
                                        .asprintf("%02X",
                                                 (uchar)infofield[k * 12 + j]));
              }
              //                        decline+=((QString)"").sprintf(" rec =
              //                        %04X calc = %04X", crc_rec,crc_calc);
              if (crc_calc == crc_rec) {
                ...
                case AES_system_table_broadcast_GES_Psmc_and_Rsmc_channels_COMPLETE:
                  decline += "AES_system_table_broadcast_GES_Psmc_and_Rsmc_"
                             "channels_COMPLETE";
                  {
                    ...
                    if (lsu <= 1) {
                      freq2 += 101.5;
                      freq3 += 101.5;
                      decline += ((QString) " Seq = %1 ").arg(seqno) +
                                 (("GES = " + (((QString) "%1")
                                                   .arg(ges, 2, 16, QChar('0'))
                                                   .toUpper())) +
                                  ((QString) " --> Psmc  = %1MHz (RX), Rsmc0 = "
                                             "%2MHz (TX), Rsmc1 = %3MHz (TX)")
                                      .arg(freq1, 0, 'f', 4)
                                      .arg(freq2, 0, 'f', 4)
                                      .arg(freq3, 0, 'f', 4));
                    } else if (lsu == 2) {
                      ...
                      decline += ((QString) " Seq = %1 ").arg(seqno) +
                                 (("GES = " + (((QString) "%1")
                                                   .arg(ges, 2, 16, QChar('0'))
                                                   .toUpper())) +
                                  ((QString) " --> Rsmc2 = %1MHz (TX), Rsmc3 = "
                                             "%2MHz (TX), Rsmc4 = %3MHz (TX)")
                                      .arg(freq1, 0, 'f', 4)
                                      .arg(freq2, 0, 'f', 4)
                                      .arg(freq3, 0, 'f', 4));
                    } else if (lsu == 3) {
                      ...
                      decline += ((QString) " Seq = %1 ").arg(seqno) +
                                 (("GES = " + (((QString) "%1")
                                                   .arg(ges, 2, 16, QChar('0'))
                                                   .toUpper())) +
                                  ((QString) " --> Rsmc5 = %1MHz (TX), Rsmc6 = "
                                             "%2MHz (TX), Rsmc7 = %3MHz (TX)")
                                      .arg(freq1, 0, 'f', 4)
                                      .arg(freq2, 0, 'f', 4)
                                      .arg(freq3, 0, 'f', 4));
                    }
                ...
                case AES_system_table_broadcast_GES_beam_support_COMPLETE:
                  decline +=
                      "AES_system_table_broadcast_GES_beam_support_COMPLETE";
                  break;
                ...
                case AES_system_table_broadcast_satellite_identification_COMPLETE:
                  decline += "AES_system_table_broadcast_satellite_id_COMPLETE";
                  {
                    ...
                    int channel1 =
                        ((((byte7 & 0x7F) << 8) & 0xFF00) | (byte8 & 0x00FF));
                    int channel2 =
                        ((((byte9 & 0x7F) << 8) & 0xFF00) | (byte10 & 0x00FF));
                    ...
                    if (byte7 & 0x80)
                      spotbeam1qstr = " (Spot beam)";
                    if (byte9 & 0x80)
                      spotbeam2qstr = " (Spot beam)";
                    ...
                    if (longitude > 180.0)
                      longitude_qstr = (QString("%1W")).arg(360.0 - longitude);
                    else
                      longitude_qstr = (QString("%1E")).arg(longitude);
                    if (channel2 != 0)
                      decline +=
                          ((QString) " SATELLITE ID = %1 (Long %3) Seq = %2 "
                                     "Psmc1 = %4MHz%6 Psmc2 = %5MHz%7")
                              .arg(satid)
                              .arg(seqno)
                              .arg(longitude_qstr)
                              .arg(cac_freq1, 0, 'f', 4)
                      ...
                      decline += ((QString) " SATELLITE ID = %1 (Long %3) Seq "
                                            "= %2  Psmc1 = %4MHz%5")
                                     .arg(satid)
                                     .arg(seqno)
                                     .arg(longitude_qstr)
                                     .arg(cac_freq1, 0, 'f', 4)
                                     .arg(spotbeam1qstr);
                    // if(channel2!=0)decline+=((QString)" SATELLITE ID = %1
                    // (Long %3 RA %6 INC %7) Seq = %2 Psmc1 = %4MHz Psmc2 =
                    // %5MHz").arg(satid).arg(seqno).arg(longitude_qstr).arg(cac_freq1,0,
                    // 'f', 3).arg(cac_freq2,0, 'f', 3).arg(ra).arg(inc);
                    //  else decline+=((QString)" SATELLITE ID = %1 (Long %3 RA
                    //  %5 INC %6) Seq = %2  Psmc1 =
                    //  %4MHz").arg(satid).arg(seqno).arg(longitude_qstr).arg(cac_freq1,0,
                    //  'f', 3).arg(ra).arg(inc);
                  }
                  break;
                ...
                case C_channel_assignment_distress:
                  decline += "C_channel_assignment_distress";
                  {
                    CChannelAssignmentItem item =
                        CreateCAssignmentItem(infofield.mid(k * 12, 12));
                ...
                case P_R_channel_control_ISU:
                  decline += "P_R_channel_control_ISU";
                  {
                    ...
                    // decline+=((" GES = "+(((QString)"%1").arg(ges, 2, 16,
                    // QChar('0')).toUpper()))+((QString)" Pd = %1MHz at %2bps
                    // with %3 frequencies assigned").arg(freq,0, 'f',
                    // 3).arg(bitrate).arg(nooffreqs));
                    if (spotbeam)
                      decline += ((" GES = " + (((QString) "%1")
                                                    .arg(ges, 2, 16, QChar('0'))
                                                    .toUpper())) +
                                  ((QString) " Pd = %1MHz at %2bps (Spot beam)")
                                      .arg(freq, 0, 'f', 3)
                                      .arg(bitrate));
                    else
                      decline += ((" GES = " + (((QString) "%1")
                ...
                case User_data_ISU_RLS_P_T_channel:
                  ...
                  {
                    /*quint32
                    AESID=((uchar)infofield[k*12+1])<<8*2|((uchar)infofield[k*12+2])<<8*1|((uchar)infofield[k*12+3])<<8*0;
                    uchar GESID=(uchar)infofield[k*12+4];
                    decodedbytes+=((QString)" AESID = %1 GESID =
                    %2").arg(AESID).arg(GESID);*/
                ...
                default:
                  if ((message & 0xC0) == 0xC0) {
                    ...
                    if (isudata.update(infofield.mid(k * 12, 10))) {
                      ...
                      parserisu->parse(
                          isudata
                              .lastvalidisuitem); // emits signals if it find
                                                  // valid acars data or errors
                    } else if (isudata.missingssu)
                      decline +=
                          " missing (or if unimplimented then TODO in C++)";

                    /*                                    decodedbytes+="SUBSEQUENT
                SIGNAL UNITS"; decodedbytes+=" \""; for(int m=2;m<10;m++)
                {
...
*/
              ...
              /*if(crc_calc==crc_rec)qDebug()<<k<<((QString)"").sprintf("rec =
           %02X", crc_rec)<<((QString)"").sprintf("calc = %02X",
           crc_calc)<<"OK"<<unencoded_BER_estimate*100.0; else
           {
              allgood=false;
              qDebug()<<k<<((QString)"").sprintf("rec = %02X",
           crc_rec)<<((QString)"").sprintf("calc = %02X", crc_calc)<<"Bad
           CRC"<<unencoded_BER_estimate*100.0;
           }
           */
            ...
            // if(!allgood)decodedbytes+="Got at least one bad SU (a SU failed
            // CRC check)\n";//tell the console
          }
        }
      }
    ...
    if (gotsync) {
      ...
      if (!burstmode) {
        if (cntr + 1 != AERO_SPEC_TotalNumberOfBits) {
          isudata.reset();
          decodedbytes += "Error short frame!!! maybe the soundcard dropped "
                          "some sound card buffers\n";
        }
      }

      //            decodedbytes+=((QString)"Bits for frame =
      //            %1\n").arg(cntr+1);
      cntr = -1;
...
CChannelAssignmentItem AeroL::CreateCAssignmentItem(QByteArray su) {
  ...
  default:
    ...
    // return item; // dont return in case this is a reserved type. however
    // without this return I wouldn't have figured out that there was a bug
    // calling this function.
  }

  item.type = (uchar)su[0];

  item.AESID = ((uchar)su[-1 + 2]) << 8 * 2 | ((uchar)su[-1 + 3]) << 8 * 1 |
               ((uchar)su[-1 + 4]) << 8 * 0;
  item.GESID = su[-1 + 5];
  ...
  if (byte7 & 0x80)
    item.receive_spotbeam = true;
  if (byte9 & 0x80)
    item.transmit_spotbeam = true;

  // looking up the db for plane info is a hassle so i'm not doing it at the
  // moment. probably something to look at later
...
void AeroL::SendCAssignment(int k, QString decline) {
  ACARSItem item;
  item.isuitem.AESID = ((uchar)infofield[k * 12 - 1 + 2]) << 8 * 2 |
                       ((uchar)infofield[k * 12 - 1 + 3]) << 8 * 1 |
                       ((uchar)infofield[k * 12 - 1 + 4]) << 8 * 0;
  ...
  for (int i = 0; i < bits.size(); i++) {
    ...
    if (gotsync) {
      ...
      if (cntr == AERO_SPEC_NumberOfBits - 1) {
        ...
        puncturedCode.depunture_soft_block(deleaveredBlock, depuncturedBlock, 4,
                                           true);

        QVector<int> deconvol =
            jconvolcodec->Decode_Continuous(depuncturedBlock);
        ...
        for (int y = 0; y < 24; y++) {
          ...
          if (infofield.size() == 12) {
            ...
            quint16 crc_rec =
                (((uchar)infofield[12 - 1]) << 8) | ((uchar)infofield[12 - 2]);
            ...
            if (crc_calc == crc_rec) {
              ...
              if (datacdcountdown > 0)
                datacdcountdown -= 5; // 3;//C channel packets come slower so
                                      // make timeout faster to compensate
            }
            if (!datacd && datacdcountdown > 2) {
              datacd = true;
              emit DataCarrierDetect(datacd);
              ...
              case Call_progress: {
                ...
                for (int k = 0; k < infofield.size() - 2; k++)
                  decline +=
                      ((QString) " 0x%1")
                          .arg(((QString) "")
                                   .asprintf("%02X", (uchar)infofield[k]));
        ...
        // 25 primary fields. this is where the audio lives in a compressed
        // format
        for (int i = 0; i < 25; i++) {
...
void PuncturedCode::depunture_soft_block(QByteArray &sourceblock,
                                         QByteArray &targetblock, int pattern,
                                         bool reset) {
  assert(pattern >= 2);
  ...
  for (int i = 0; i < sourceblock.size() - 1; i++) {
    ...
    if (depunture_ptr >= pattern - 1)
      targetblock.push_back(128);
