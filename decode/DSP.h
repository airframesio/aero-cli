...
public:
  ...
  double GetFreqTest();
  void Advance(double FractionOfSample) {
    WTptr += FractionOfSample * WTstep;
    if (((int)WTptr) >= WTSIZE)
      WTptr -= WTSIZE;
    if (((int)WTptr) < 0)
      WTptr += WTSIZE;
  }
  void AdvanceFractionOfWave(double FractionOfWave) {
  ...
  void Retard(double FractionOfSample) {
    WTptr -= FractionOfSample * WTstep;
    if (((int)WTptr) >= WTSIZE)
      WTptr -= WTSIZE;
    if (((int)WTptr) < 0)
...
template <class T> class TMovingAverage {
public:
...
public:
  void design(double alpha, int firsize, double samplerate,
              double symbol_freq) {
    if ((firsize % 2) == 0)
      firsize += 1;
    ...
    for (int i = 0; i < firsize; i++) {
      ...
      if (i == ((firsize - 1) / 2))
        ...
        if (fabs(1.0 - pow(4.0 * alpha * fi / T, 2)) < 0.0000000001)
          Points[i] = (alpha *
                       ((M_PI - 2.0) * cos(M_PI / (4.0 * alpha)) +
                        (M_PI + 2.0) * sin(M_PI / (4.0 * alpha))) /
                       (M_PI * sqrt(2.0 * T)));
        else
          Points[i] =
              (4.0 * alpha / (M_PI * sqrt(T)) *
               (cos((1.0 + alpha) * M_PI * fi / T) +
                T / (4.0 * alpha * fi) * sin((1.0 - alpha) * M_PI * fi / T)) /
               (1.0 - pow(4.0 * alpha * fi / T, 2)));
      }
    }
  }
...
public:
  ...
  T update(T sig) {
    ...
    while (std::floor(dptr) < 0)
      dptr += ((double)buff.size());
...
template <class T> class Intergrator {
public:
  Intergrator() { setlength(1); }
  void setlength(int length) {
    MASz = length;
    MABuffer.resize(MASz);
    for (int i = 0; i < MABuffer.size(); i++)
      MABuffer[i] = (T)0;
    MAPtr = 0;
    Val = 0;
  }
...
template <class T> class DelayThing {
public:
  DelayThing() { setLength(12); }
  void setLength(int length) {
    length++;
    assert(length > 0);
...
public:
  PeakDetector() {
    setSettings((int)(9.14 * 128.0 / 2.0),
                0.25); // not sure what the best length should be
  }
  ~PeakDetector() {}
  ...
  bool update(double &val) {
    ...
    if ((!cntdown) && (val > threshold) &&
        ((lastdy >= 0 && dy < 0))) // clear, going up and above threshold
    {
