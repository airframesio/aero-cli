...
OqpskDemodulator::OqpskDemodulator(QObject *parent) : QIODevice(parent) {
  ...
  connect(this, SIGNAL(BBOverlapedBuffer(const QVector<cpx_type> &)),
          coarsefreqestimate,
          SLOT(ProcessBasebandData(const QVector<cpx_type> &)));
  connect(coarsefreqestimate, SIGNAL(FreqOffsetEstimate(double)), this,
          SLOT(FreqOffsetEstimateSlot(double)));
...
void OqpskDemodulator::setSettings(Settings _settings) {
  ...
  if (_settings.fb != fb)
    emit BitRateChanged(_settings.fb, false);
  fb = _settings.fb;
  ...
  bbcycbuff_ptr = 0;
  bbtmpbuff.resize(bbnfft);
  coarsefreqestimate->setSettings(_settings.coarsefreqest_fft_power,
  ...
  RootRaisedCosine rrc;
  if (fb == 8400)
    rrc.design(0.6, 55, Fs, fb / 2);
  else
    rrc.design(1.0, 55, Fs, fb / 2);
  fir_re = new FIR(rrc.Points.size());
  fir_im = new FIR(rrc.Points.size());
  for (int i = 0; i < rrc.Points.size(); i++) {
    fir_re->FIRSetPoint(i, rrc.Points.at(i));
  ...
  // the rrc filter for 8400bps probably could be used for 10500 but the 10500
  // works well enough. maybe another type of filter would be better?
  RootRaisedCosine rrc_pre_imp;
  if (fb == 8400)
    rrc_pre_imp.design(0.6, 2048, Fs,
                       fb / 2); // 0.6 --> smaller number mean less interchannel
                                // interference but locking is harder
  else
    rrc_pre_imp.design(1.0, 2048, Fs, fb / 2);
  fir_pre.SetKernel(
      rrc_pre_imp.Points,
      4096); // rrc_pre_imp.Points.size()*2);//use x2 rather than the x4 rule of
             // thumb, will make it more responsive but may use more cpu
...
void OqpskDemodulator::CenterFreqChangedSlot(
    double freq_center) // spectrum display calls this when user changes the
                        // center freq
{
  if (fb != 8400) // ppl are going to shift the signal too far left or right as
                  // alpha<1.0
  {
    if (freq_center < (0.5 * fb))
      freq_center = 0.5 * fb;
    if (freq_center > (Fs / 2.0 - 0.5 * fb))
      freq_center = Fs / 2.0 - 0.5 * fb;
  }
  mixer_center.SetFreq(freq_center, Fs);
  if (afc)
...
qint64 OqpskDemodulator::writeData(const char *data, qint64 len) {
  ...
  for (i = 0; i < (int)(len / sizeof(short)); i++) {
    ...
    double dabval =
        std::sqrt(sig2.real() * sig2.real() + sig2.imag() * sig2.imag());
    ...
    if (st_osc.IfHavePassedPoint(ee)) {
      ...
      if (!yui)
        ...
        // 4 power method
        // cpx_type
        // trick_pt=std::pow(pt_qpsk*cpx_type(cos(M_PI_4),sin(M_PI_4)),4);
  ...
  // update the 8400bps pre filter with better estimates of carrier in case
...
void OqpskDemodulator::FreqOffsetEstimateSlot(
    double freq_offset_est) // coarse est class calls this with current est
{
  ...
  if ((mse < signalthreshold) &&
      (!dcd)) // signal but we arent getting data so prob in a state that is
              // stable but wrong
  {
    if (countdown2 > 0)
      countdown2--;
  ...
  if ((mse > signalthreshold) &&
      (fabs(mixer2.GetFreqHz() - (mixer_center.GetFreqHz() + freq_offset_est)) >
       3.0)) // no sig, prob cant track carrier phase
  {
    mixer2.SetFreq(mixer_center.GetFreqHz() + freq_offset_est);
    // symtracker.Reset();
  }
  if ((afc) && (mse < signalthreshold) &&
      (fabs(mixer2.GetFreqHz() - mixer_center.GetFreqHz()) >
       3.0)) // got a sig, afc on, freq is getting a little to far out
  {
    if (countdown > 0)
      countdown--;
    else {
...
void OqpskDemodulator::DCDstatSlot(bool _dcd) { dcd = _dcd; }
