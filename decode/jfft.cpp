...
void JFFT::init(int &fft_size) {
  ...
  // alocate mem
  TWIDDLE_mem.resize(nfft); // this is more memory than we need. what is the
  ...
  // the pattern is 2 4 8 16 32 .. for the bottom number (cake cut into this
  // many piecies N) and the top number increases till not quite half way around
  // the cake (ie less than N/2)
  ...
  for (int N = 2; N <= nfft; N <<= 1) {
    for (int i = 0; i < N / 2; i++) {
      cpx_type twiddle =
          std::exp(-2.0 * imag * M_PI * ((double)i) / ((double)N));
      cpx_type twiddle_inv =
          std::exp(2.0 * imag * M_PI * ((double)i) / ((double)N));
      assert(w < nfft);
      TWIDDLE[w] = twiddle;
  ...
  for (int i = 0; i < nfft; i++) {
    DIDDLE_A[i] =
        0.5 * (1.0 - imag * std::exp(-2.0 * imag * M_PI * ((double)i) /
                                     ((double)(2 * nfft))));
    DIDDLE_B[i] =
        0.5 * (1.0 + imag * std::exp(-2.0 * imag * M_PI * ((double)i) /
                                     ((double)(2 * nfft))));
...
void JFFT::ifft_real(const cpx_type *complex, double *real, int size) {
  ...
  // do the diddling
  for (int i = 0; i < nfft; ++i) {
    F[i] = complex[i] * std::conj(DIDDLE_A[i]) +
           std::conj(DIDDLE_B[i]) * std::conj(complex[(nfft - i)]);
  }
...
void JFFT::fft(cpx_type *x, int size, fft_direction_t fft_direction) {
  ...
  */
  ...
  // fft. using pointers harder to understand but is identical to the one in
  // fft_easy_to_understand
  int nfill = 0;
  cpx_type y;
  cpx_type *xkp;
  cpx_type *xlp;
...
void JFFT::fft_easy_to_understand(cpx_type *x, int size,
                                  fft_direction_t fft_direction) {
  assert(size == nfft);
  cpx_type *TWIDDLE;
  if (fft_direction == FORWARD)
    TWIDDLE = TWIDDLE_mem.data();
  ...
  // http://www.alwayslearn.com/DFT%20and%20FFT%20Tutorial/DFTandFFT_FFT_Butterfly_8_Input.html
  // k points to top part of a buterfly and l points to the bottom part of a
  // buterfly. w points to the twiddle that is currently needed. n is for how
  // many buterfly are in a set, stage 1 has 1 stage 2 has 2 stage 3 has 4 stage
  // 4 has 8 and so on.
...
// DFT from definition
// if you were really wanting the best from it you should move Wf to the init
// function but as this is just a rough comparison between a fft and a slow ft
// implimentation this should do
void JFFT::sft(cpx_type *x, int size, fft_direction_t fft_direction) {
  cpx_type imag = cpx_type(0, 1);
  cpx_type W;
  F.assign(size, 0);
  if (fft_direction == FORWARD)
    W = std::exp(-2.0 * imag * M_PI / ((double)size));
  else
    W = std::exp(2.0 * imag * M_PI / ((double)size));
...
void JFastFir::SetKernel(const JFFT::cpx_type *_kernel, int kernel_size,
                         int approx_fft_size) {
  // copy kernel over
  kernel.resize(kernel_size);
  memcpy(kernel.data(), _kernel, sizeof(JFFT::cpx_type) * kernel_size);

  // use a bigger FFT size at least 4 x the size of the kernel and make it a
  // power of 2
  kernel_non_zero_size = kernel.size();
  nfft = 1;
  if (approx_fft_size <= 0)
    approx_fft_size = 4 * kernel_non_zero_size; // rule of thumb
...
void JFastFir::update_block(JFFT::cpx_type *buffer, int size) {
  ...
  while (samples_processed < size) {
    ...
    if (sigspace_ptr >= signal_non_zero_size) {
      ...
      if (signal_non_zero_size <= 0)
        return; // check if the fastfir has been initalized. if it hasn't just
                // return what ever we get sent
      ...
      psigspace_overlap =
          psigspace +
          signal_non_zero_size; // pointer to start of the overlap in sigspace
      for (int k = 0; k < remainder_size; ++k) {

        *psigspace += *premainder;        // add last overlap to this sigspace
        *premainder = *psigspace_overlap; // save the remainder from this
                                          // convolution to remainder
        *psigspace_overlap =
            0; // the sigspace needs to be padded with zeros once again
    ...
    // calculate the maximum amount of samples we can copy over, that is either
    // the number of samples we still have to process (size-samples_processed)
    // or the number of samples till we fill sigspace
    // (signal_non_zero_size-sigspace_ptr).
    int number_to_copy_over =
        std::min(signal_non_zero_size - sigspace_ptr, size - samples_processed);
    // qDebug()<<number_to_copy_over;
    memcpy(tmp_space.data(), &buffer[samples_processed],
           sizeof(JFFT::cpx_type) *
               number_to_copy_over); // take the unprocessed samples from the
                                     // buffer and put them in tmp space
    memcpy(&buffer[samples_processed], &sigspace[sigspace_ptr],
           sizeof(JFFT::cpx_type) *
               number_to_copy_over); // take the processed samples and put them
                                     // into the buffer
    memcpy(&sigspace[sigspace_ptr], tmp_space.data(),
           sizeof(JFFT::cpx_type) *
               number_to_copy_over); // take the samples from the tmp space and
                                     // put them into the space for processing
...
JFFT::cpx_type JFastFir::update(JFFT::cpx_type in_val) {
  ...
  if (sigspace_ptr >= signal_non_zero_size) {
    ...
    if (signal_non_zero_size <= 0)
      return in_val; // check if the fastfir has been initalized. if it hasn't
                     // just return what ever we get sent
...
JFFT::cpx_type JFastFir::update_easy_to_understand(JFFT::cpx_type in_val) {
  ...
  if (sigspace_ptr >= signal_non_zero_size) {
    ...
    // data has increased from N to N+M-1 (N is signal size and M is kernel
    // size). we have set it up so N+M-1==nfft and the last M-1 are saved for
    // next time in the remainder buffer. The M-1 samples from the previous time
    // are added to the start of this time. Finally we padd the next signal with
    // zeros to avoid time aliasing. it sonds confusing but it's really not as
    // bad as it sounds.

    // add remainder from last time to the start of this one
    for (int k = 0; k < remainder_size; ++k)
      sigspace[k] += remainder[k];

    // save the remainder of this time for the next one
    for (int k = 0; k < remainder_size; ++k)
      remainder[k] = sigspace[k + signal_non_zero_size];

    // clear the end of this for the next fft
    for (int k = 0; k < remainder_size; ++k)
      sigspace[k + signal_non_zero_size] = 0;
...
// this could be a bit faster but it's easier to understand this way and the
// loss of speed is not much
double JFastFir::update(double real_in) {
  // if we are back at zero then time for an fft
  if (sigspace_ptr >= signal_non_zero_size) {

    if (signal_non_zero_size <= 0)
      return real_in; // check if the fastfir has been initalized. if it hasn't
                      // just return what ever we get sent
    ...
    for (int k = 0; k < (nfft / 2 + 1); ++k)
      sigspace[k] *=
          kernel[k]; // as it's real only slightly over half of freq is needed
    ...
    // bad as it sounds.
    for (int k = 0; k < remainder_size; ++k) {
      sigspace_real[k] += remainder_real[k]; // add remainder from last time to
                                             // the start of this one
      remainder_real[k] =
          sigspace_real[k + signal_non_zero_size]; // save the remainder of this
                                                   // time for the next one
      sigspace_real[k + signal_non_zero_size] =
          0; // clear the end of this for the next fft
    }
...
std::vector<JFFT::cpx_type>
JFilterDesign::LowPassHanning(double FrequencyCutOff, double SampleRate,
                              int Length) {
  ...
  for (int i = (-(Length - 1) / 2); i <= ((Length - 1) / 2); i++) {
    double w =
        0.5 * (1.0 - std::cos(2.0 * M_PI * ((double)j) / ((double)(Length))));
    h.push_back(
        w * (2.0 * FrequencyCutOff / SampleRate) *
        sinc_normalized(2.0 * FrequencyCutOff * ((double)i) / SampleRate));
    j++;
  }
  ...
  hideal =
  (2*FrequencyCutOff/SampleRate)*sinc(2*FrequencyCutOff*idx/SampleRate); h =
  hanning(Length)' .* hideal;
  */
}

std::vector<JFFT::cpx_type>
JFilterDesign::HighPassHanning(double FrequencyCutOff, double SampleRate,
                               int Length) {
  std::vector<JFFT::cpx_type> h;
  if (Length < 1)
    return h;
...
std::vector<JFFT::cpx_type>
JFilterDesign::BandPassHanning(double LowFrequencyCutOff,
                               double HighFrequencyCutOff, double SampleRate,
                               int Length) {
  std::vector<JFFT::cpx_type> h;
  if (Length < 1)
    return h;
...
std::vector<JFFT::cpx_type>
JFilterDesign::BandStopHanning(double LowFrequencyCutOff,
                               double HighFrequencyCutOff, double SampleRate,
                               int Length) {
  std::vector<JFFT::cpx_type> h;
  if (Length < 1)
  ...
  h1 = BandPassHanning(LowFrequencyCutOff, HighFrequencyCutOff, SampleRate,
                       Length);
  if ((h1.size() == (size_t)Length) && (h2.size() == (size_t)Length)) {
    for (int i = 0; i < Length; i++)
      h.push_back(h2[i] - h1[i]);
