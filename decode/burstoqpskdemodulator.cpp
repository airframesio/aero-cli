...
BurstOqpskDemodulator::BurstOqpskDemodulator(QObject *parent)
  ...
  ebnomeasure = new OQPSKEbNoMeasure(SamplesPerSymbol * (256.0), Fs,
                                     fb); // 256 symbol ave, Fs and fb
...
void BurstOqpskDemodulator::setSettings(Settings _settings) {
  ...
  d1.setLength((int)(SamplesPerSymbol * 128.0 * 2.5 -
                     190)); // adjust so start of burst aligns
...
double BurstOqpskDemodulator::getCurrentFreq() { return mixer2.GetFreqHz(); }

void BurstOqpskDemodulator::CenterFreqChangedSlot(
...
void BurstOqpskDemodulator::writeDataSlot(const char *data, qint64 len) {
  ...
  for (int i = 0; i < hfirbuff.size(); i++) {
                                 ...
                                 // amplitude
    {
      ...
      for (int k = qRound(128.0 * SamplesPerSymbol); k < in.size(); k++)
        in[k] = 0;
      fftr->transform(in, out_base);

      // top
      in = tridentbuffer.mid(qRound(128.0 * SamplesPerSymbol),
                             qRound(128.0 * SamplesPerSymbol));
      ...
      double maxval = out_abs_diff[firstbin - binpeakspacing_int] +
                      out_abs_diff[firstbin + binpeakspacing_int] -
                      out_abs_diff[firstbin];
      double maxvalbin = firstbin;
      for (int i = firstbin; i < lstbin; i++) {
        assert(i < out_abs_diff.size());
        double testval = out_abs_diff[i - binpeakspacing_int] +
                         out_abs_diff[i + binpeakspacing_int] - out_abs_diff[i];
        if (testval > maxval) {
          maxval = testval;
          maxvalbin = i;
        }
      ...
      // check that signal is strong enough and two locs are close else this is
      // not a signal
      if ((maxval > 500.0) &&
          (fabs((((double)(maxvalbin - minvalbin))) * hzperbin) < 20.0)) {

        // qDebug()<<"vaxval="<<maxval<<" maxvalhz1="<<hzperbin*maxvalbin<<"
        // maxvalhz2="<<hzperbin*minvalbin<<"Hz"<<"
        // maxvalhz2b="<<hzperbin*minvalbin2<<"Hz";
    ...
    // rrc
    cpx_type sig2 = cpx_type(fir_re->FIRUpdateAndProcess(cval_dd.real()),
                             fir_im->FIRUpdateAndProcess(cval_dd.imag()));
    ...
    if ((cntr > SamplesPerSymbol * (128 + 10)) &&
        (cntr < ((256 - 10) * SamplesPerSymbol))) {

      // 0 to 1
      double progress =
          (((double)cntr) - (SamplesPerSymbol * (128 + 10))) /
          (((256 - 10) * SamplesPerSymbol) - (SamplesPerSymbol * (128 + 10)));
      ...
      symboltone_averotator =
          symboltone_averotator * 0.95 + 0.05 * symboltone_rotator;
      symboltone_pt =
      ...
      // x4 pll
      double st_err =
          std::arg((st_osc_quarter.WTCISValue()) * std::conj(symboltone_pt));
      st_err *= 1.5 * (1.0 - progress * progress);
      st_osc_quarter.AdvanceFractionOfWave(-(1.0 / (2.0 * M_PI)) * st_err *
                                           0.1);
    ...
    if (fabs(cntr - ((128.0 + 128.0 + 128.0) * SamplesPerSymbol)) < 0.5)
    ...
    if (cntr > SamplesPerSymbol * (128 + 128))
    ...
    if (st_osc.IfHavePassedPoint(ee)) //?? 0.4 0.8 etc
    {
      ...
      double twospeed =
          -4.0 *
          ((std::fmod((st_osc_quarter.GetPhaseDeg()) * 2.0 + (360.0 * ee * 0.5),
                      360.0) /
            360.0) -
           (0.34046 + 0.4111 * ee));
      bool even = true;
      ...
      if (!yui)
        ...
        {
          rotator =
              rotator * std::exp(imag * ct_ec * 0.1); // correct carrier phase
          if (cntr > ((128 + 10) * SamplesPerSymbol))
            rotator_freq =
        ...
        /*cpx_type
        trick_pt=std::pow(pt_qpsk*cpx_type(cos(M_PI_4),sin(M_PI_4)),4); double
        fang=std::arg(trick_pt)/4.0; rotation_bias_ma->UpdateSigned(fang);
        ...
        if (pointbuff_ptr > 0) {
          ...
          if (scatterpointtype == SPT_constellation &&
              (pointbuff_ptr == pointbuff.size()) && (sendscatterpoints)) {
            pointbuff_ptr++;
        ...
        if (scatterpointtype == SPT_phaseoffsetest) {
          ...
          if (fabs(cntr - ((128.0 + 128.0 + 128.0) * SamplesPerSymbol)) <
              SamplesPerSymbol) {
            cpx_type st_phase_offset_pt =
                st_osc_ref.WTCISValue() * std::conj(st_osc.WTCISValue());
