...
void JConvolutionalCodec::SetCode(int inv_rate, int order,
                                  const QVector<quint16> poly,
                                  int _paddinglength) {
...
QVector<int> &
JConvolutionalCodec::Decode_hard(const QByteArray &soft_bits_in,
                                 int size) // 0-->-1 128-->0 255-->1
{
  ...
  size_t dbits = correct_convolutional_decode(
      convol, (uchar *)soft_bits_overlap_buffer_uchar.data(), bit_counter,
      (uchar *)decoded.data());
  assert(dbits > 0);
  ...
  for (int i = 0; i < decoded.size() && bit_ptr < ((int)dbits); i++) {
    ...
    for (int k = 0; k < 8; k++) {
      if (uch & 128)
        decoded_bits[bit_ptr] = 1;
      else
        decoded_bits[bit_ptr] = 0;
      uch <<= 1;
      bit_ptr++;
    }
  }
...
QVector<int> &
JConvolutionalCodec::Decode_soft(QByteArray &soft_bits_in,
                                 int size) // 0-->-1 128-->0 255-->1
{
  ...
  size_t dbits = correct_convolutional_decode_soft(
      convol, (uchar *)soft_bits_overlap_buffer_uchar.data(), size,
      (uchar *)decoded.data());
  assert(dbits > 0);
...
QVector<int> &
JConvolutionalCodec::Soft_To_Hard_Convert(const QByteArray &soft_bits_in) {
  decoded_bits.clear();
  for (int i = 0; i < soft_bits_in.size(); i++) {
    if (((uchar)(soft_bits_in.at(i))) >= 128)
...
QByteArray &
JConvolutionalCodec::Hard_To_Soft_Convert(QByteArray &hard_bits_in) {
...
QVector<int> &JConvolutionalCodec::Decode_Continuous(
    QByteArray &soft_bits_in) // 0-->-1 128-->0 255-->1
{
  ...
  size_t dbits = correct_convolutional_decode_soft(
      convol, (uchar *)soft_bits_overlap_buffer_uchar.data(),
      soft_bits_overlap_buffer_uchar.size(), (uchar *)decoded.data());
  assert(dbits > 0);
  ...
  decoded_bits =
      decoded_bits.mid(paddinglength + 1, soft_bits_in.size() / nparitybits);
...
QVector<int> &JConvolutionalCodec::Decode_Continuous_hard(
    const QByteArray &soft_bits_in) // 0-->-1 128-->0 255-->1
{
  int k = (2 * nparitybits * paddinglength) /
          8; // msg is padded frount and back both times by paddinglength
  if (soft_bits_overlap_buffer_uchar.size() != k)
    soft_bits_overlap_buffer_uchar.fill(0, k);
  ...
  size_t dbits = correct_convolutional_decode(
      convol, (uchar *)soft_bits_overlap_buffer_uchar.data(),
      bit_counter + k * 8, (uchar *)decoded.data());
  assert(dbits > 0);
  ...
  // remove the padding bits from the return data
  decoded_bits = decoded_bits.mid(
      paddinglength); //(paddinglength/nparitybits);//-(constraint-1)+1);
