...
public:
  enum win_type {
    ...
    WIN_KAISER =
        4, //!< Kaiser window; max attenuation a function of beta, google it
  ...
  QVector<float> low_pass(double gain, double sampling_freq, double cutoff_freq,
                          double transition_width, win_type window_type,
                          double beta);
...
private:
  ...
  void sanity_check_1f(double sampling_freq, double fa,
                       double transition_width);
  int compute_ntaps(double sampling_freq, double transition_width,
