/*this code comes from gnuradio which is covered by the GNU GENERAL PUBLIC
LICENSE
...
QVector<float>
firfilter::low_pass(double gain, double sampling_freq,
                    ...
                    double beta) // used only with Kaiser
{
  ...
  auto wv = window(window_type, ntaps);
  QVector<float> w(wv.begin(), wv.end());
...
int firfilter::compute_ntaps(double sampling_freq, double transition_width,
...
double firfilter::max_attenuation(win_type type, double beta) {
  ...
  default:
    throw std::out_of_range(
        "window::max_attenuation: unknown window type provided.");
  }
}
...
std::vector<float> firfilter::blackman_harris(int ntaps, int atten) {
  ...
  default:
    throw std::out_of_range(
        "window::blackman_harris: unknown attenuation value "
...
std::vector<float> firfilter::coswindow(int ntaps, float c0, float c1,
                                        float c2) {
  std::vector<float> taps(ntaps);
  float M = static_cast<float>(ntaps - 1);
